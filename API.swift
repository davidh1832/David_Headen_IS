...
class ChatbotAPI {
    ...
    private let baseURL = "http://127.0.0.1:8000" // url for server
    ...
    func getReligiousQuote(userInput: String, source: String) async throws -> QuoteResponse {
        ...
        return try JSONDecoder().decode(QuoteResponse.self, from: data) // Json data -> struct (Quote, chapter, source)
    }
}
