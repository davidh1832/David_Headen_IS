...
// This code is the front-end API Integration, which enables asynchronous calls between front-end 
class ChatbotAPI {
    
    private let baseURL = "http://127.0.0.1:8000" // Uvicorn Server
    ...
    func getChatbotResponse(userInput: String, userID: String) async throws -> String {
        ...
        // send user input and user id to backend
        ...
        // Start round trip timer for time analysis
        let startTime = Date()
        ...
        //Return mental health response from mental health chat pipeline
    ...
    func getReligiousQuote(userInput: String, source: String) async throws -> QuoteResponse {
        ...
        // Pass in user input and the source chosen by the user
        let body: [String: Any] = [
            "user_input": userInput,
            "source": source
        ]
        ...
        return try JSONDecoder().decode(QuoteResponse.self, from: data) // Data from religious quote pipeline
    }
}
